class TimeDeposit:
    ...
    def __str__(self):
        ...
        Сумма:              [1,000; 100,000)
        """
        return ('{0[0]:<20}{1.name}\n\
{0[1]:<20}{1.currency}\n\
{0[2]:<20}{1._interest_rate}\n\
{0[3]:<20}[{1._period_limit[0]}; {1._period_limit[1]})\n\
{0[4]:<20}[{1._sum_limit[0]:,}; {1._sum_limit[1]:,})'.
                format(('Наименование:', 'Валюта:',
                        'Процентная ставка:', 'Срок (мес.):', 'Сумма:'), self))
...
class BonusTimeDeposit(TimeDeposit):
    ...
    def __str__(self):
        ...
        return super().__str__() +\
            ('\n{0[0]:<20}{1[percent]}\n{0[1]:<20}{1[sum]:,}'.
             format(('Бонус (%):', 'Бонус (мин. сумма):'), self._bonus))
    ...
    def _check_self(self):
        ...
        """
        super()._check_self
        assert isinstance(self._bonus, dict), \
            'Бонус - не словарь!'
        assert len(self._bonus) == 2, \
            'Неверный формат бонуса!'
        assert isinstance(self._bonus['percent'], int), \
            'Бонусные проценты - не  целое число!'
        assert isinstance(self._bonus['sum'], (float, int)), \
            'Минимальная бонусная сумма - не число!'
        assert self._sum_limit[0] <= self._bonus['sum']\
            < self._sum_limit[1], \
            'Неверно указана минимальная бонусная сумма!'
        assert 0 < self._bonus['percent'] <= 100, \
            'Неверно указан бонусный процент!'
    ...
    BonusTimeDeposit(
        ...
        **deposits_data,
        bonus={
            'percent': 100,
            'sum': 3000}),
    CompoundTimeDeposit('Пол процента в месяц', interest_rate=6,
                        period_limit=(6, 36), sum_limit=(1000, 1000000)))
